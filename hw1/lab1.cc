...
int main(int argc, char* argv[])
{
  ...
  //path = "./Lab1_Checker/scenario/third.txt";
  ifstream in(path);
  if(!in.is_open()){
  ...
  for(int i=0;i<(int)consumers_rec.size();i++){
    ...
    apps.Stop(Seconds(stof(consumers_rec[i].app_duration_time)));
  }

  // Producer
  ndn::AppHelper producerHelper("ns3::ndn::Producer");
  // Producer will reply to all requests starting with /prefix
  // set producer is prefix or postfix
  for(int i=0;i<(int)producers_rec.size();i++){
